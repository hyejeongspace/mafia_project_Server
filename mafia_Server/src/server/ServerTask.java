			...
			try {
				...
				switch(code) {
				   ...
				   case "ability" :
					   	...
					   	room = findRoom(roomNumber);
					   	if(nick.equals(room.ccUser.get(0).getNickname())) {
					   		if(!room.getDoctorSelect().equals(null) && 
					   			!room.getMafiaSelect().equals(null)) {
					   			String killCheck = ability(room);
					   			if(killCheck.equals("noDie")) {
					   				roomData(room,"noKill,");
					   				roomMDClear(room);
					   			}else {
					   				findMember(killCheck).setAlive("die");
					   				roomData(room,"kill,"+killCheck);
					   				roomMDClear(room);
					   			}
					   		}else if(!room.getMafiaSelect().equals(null)) {
								String killCheck = ability(room);
				                findMember(killCheck).setAlive("die");
				                roomData(room,"kill,"+killCheck);
				                roomMDClear(room);
	...
	public void dieCast(String msg , ChatRoom room,String nick) {
		for(int i = 0 ; i < room.ccUser.size() ; i++) {
			...
			if(alive.equals("die")) {
				...
				p.println("chatDie,"+room+","+msg+","+nick);
			}
		}
	}
